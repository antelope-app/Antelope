...
class EZParseHelper:
    def split_at_char(target, char, right=True):
        separator_index = target.find(char)
        if right:
            non_inclusive_index = separator_index + 1
    ...
    def regex_for_type(ez_rule_type):
        ...
        if ez_rule_type = EZRuleType.comment:
            return '!'
        elif ez_rule_type = EZRuleType.block_domain:
            return '||'
        elif ez_rule_type = EZRuleType.block_exact
            return '|'
        elif ez_rule_type = EZRuleType.selector
            return '##'
        elif ez_rule_type = EZRuleType.exception
            return '@@'
...
class EZRuleParser:
    def parse(raw_rule):
        ...
        ez_rule.type = _type(raw_rule)
        if ez_rule.type is EZRuleType.selector:
            ez_rule.selector = _css_selector(raw_rule)

        raw_options = _raw_options(raw_rule)
        if raw_options is not None:
            ez_rule.content_options = _content_options(raw_options)
            ez_rule.load_types = _load_types(raw_options)
            ez_rule.domains = _domains(raw_options)

        ez_rule.url_filter = _url_filter(raw_rule, ez_rule_type)
    ...
    def _url_filter(raw_rule, ez_rule_type):
        ...
        if ez_rule.type is EZRuleType.selector:
            url_filter = _selector_url_filter(raw_rule)
        else:
            # To capture @@|| or @@| constructions 
            if ez_rule.type is EZRuleType.exception:
                regex = EZRuleParser.regex_for_type(ez_rule_type)

                #Casts the type to the newly pre-pended url_filter
                ez_rule_type = _type(url_filter)

            regex = EZRuleParser.regex_for_type(ez_rule_type)
            url_filter = EZParseHelper.split_at_char(raw_rule, regex)

        return url_filter

    def _selector_url_filter(raw_rule):
        return EZParseHelper.split_at_char(raw_rule, "##", right=False)
    ...
    def _raw_options(raw_rule):
        ...
        if '$' in raw_rule:
            options = EZParseHelper.split_at_char(raw_rule, '$').split(',')
    ...
    def _domains(raw_options):
        ...
        for option in raw_options:
            if _is_domain_option(option):
                domains = EZParseHelper.split_at_char(option, '=').split('|')
                break
    ...
    def _load_types(raw_options):
        ...
        for option in raw_options:
            if _is_load_type_option(option):
    ...
    def _content_options(raw_options):
        ...
        for option in raw_options:
            if not _is_domain_option(option) and /
               not _is_load_type_option(option):
...
class EZTranslator(Translator):
    ...
    def _action_type(ez_rule_type):
        if ez_rule_type = EZRuleType.selector:
